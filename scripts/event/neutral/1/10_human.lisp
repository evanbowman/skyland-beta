...
(dialog
 ...
 (tr "You come upon the scene of a heated battle. <B:0> A goblin resource harvester vessel is attacking an unarmed village, its beams slicing through the town. <B:0> Noticing you, the goblins briefly stop attacking…"))
...
(defn/temp defend-villagers ()
  ...
  (await (dialog* (tr "<c:Goblin:2>You think you can fight usss?! Hah!!")))
  ...
  (let ((slot (chr-slots (player))))
    (if slot
        (chr-new (player)
                 ...
                 'neutral
                 '((race . 0) (icon . 5)))))
  (await (dialog* (tr "One of the villagers came aboard to help out!"))))


(defn/temp decline ()
  (await (dialog* (tr "The goblins resume their attack. <B:0> ")
                  (tr "Maybe you weren't strong enough to face them anyway…")))
  (exit))
...
(defn on-converge ()
  ...
  (let ((sel (await (dialog-choice*
                     (string (tr "<c:Villager:5>Oh! Just in time! Can you help us!? That goblin ship over there is very strong, but there's a chance you could defeat them..."))
                     (tr '("Of course!"
