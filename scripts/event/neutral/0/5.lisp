...
(setq on-converge
      (lambda
        (load-dialog "zone1" "op_offer")

        (setq on-dialog-closed
              (lambda
                (load-dialog "zone1" "op_aside")
                (dialog-await-y/n)
                (setq on-dialog-closed '())))
        ...
        (let ((temp (chr-slots (player)))
          ...
          (if temp
              (progn
                ...
                (load-dialog "zone1" "op_join")
                (end))
            (progn
              (load-dialog "zone1" "op_no_room")
              (defn on-dialog-closed [0]
                (load-dialog "zone1" "op_plea")
                (defn on-dialog-closed [0]
                  (alloc-space 'cargo-bay)
                  (sel-input 'cargo-bay
                             (get-dialog "zone1" "op_place")
                             (lambda
                               (sound "build0")
                               (room-new (player) `(cargo-bay ,$1 ,$2))
                               (chr-del (opponent) 1 12)
                               (chr-new (player) $1 (+ 1 $2) 'neutral nil)
                               (load-dialog "zone1" "op_thanks")
                               (defn on-dialog-closed [0]
                                 (adventure-log-add 15 '())
                                 (load-dialog "zone1" "op_join")
                                 (end)))))))))
