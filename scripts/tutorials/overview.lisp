;;;
;;; overview.lisp
;;;


(coins-add 2300)

(terrain-set (player) 4)
(island-configure
 (player)
 '((power-core 1 13)))


(opponent-init 4 'hostile)

(island-configure
 (opponent)
 '((power-core 1 13)
   (missile-silo 0 13)))

(flag-show (opponent) 0)


;; Captured with a keylogger that I hacked into playerP1.cpp (commented out)
;;
;; NOTE: The first element in each nested list is a number of microseconds to
;; wait before interpreting the second element of each list. If the list
;; contains unevaluated code, the autopilot will evaluate it.
(autopilot
 '((2000 nil)
   (100 (lc-dialog-get "tutorials" "ov1"))
   (100 nil)
   (100 (lc-dialog-get "tutorials" "ov2"))
   (901 L-p)
   (60 L-np)
   (1007 Right)
   (1233 Right)
   (649 Right)
   (583 Right)
   (466 Right)
   (3000 nil)
   (100 (lc-dialog-get "tutorials" "ov3"))
   (999 Left)
   (1549 Left)
   (516 Left)
   (516 Left)
   (300 nil)
   (100 (lc-dialog-get "tutorials" "ov4"))
   (400 R)
   (1800 A)
   (1550 A)
   (733 Right)
   (749 A)
   (1317 A)
   (1365 Right)
   (1032 B)
   (1133 L-p)
   (50 L-np)
   (9350 nil)
   (100 (lc-dialog-get "tutorials" "ov5"))
   (584 L-p)
   (50 L-np)
   (1500 Left)
   (433 Left)
   (366 Up)
   (417 Up)
   (2000 nil)
   (100 nil)
   (100 (lc-dialog-get "tutorials" "ov6"))
   (1250 R)
   (717 Right)
   (400 Right)
   (616 A)
   (1016 Right)
   (949 A)
   (700 A)
   (599 Left)
   (933 A)
   (633 B)
   (683 Down)
   (716 Down)
   (100 nil)
   (100 (lc-dialog-get "tutorials" "ov7"))
   (733 A)
   (750 A)
   (2866 L-p)
   (50 L-np)
   (3080 Right)
   (433 Right)
   (10000 nil)
   (100 (lc-dialog-get "tutorials" "ov8"))
   (449 Left)
   (367 Left)
   (367 Left)
   (366 Left)
   (250 R)
   (450 A)
   (449 A)
   (884 B)
   (766 Right)
   (467 Right)
   (1749 Down)
   (349 Down)
   (400 Down)
   (100 nil)
   (100 (lc-dialog-get "tutorials" "ov9"))
   (2800 nil)))


;;(print (string (interp-stat)))
